...
var request = __webpack_require__(382);
...
class JWKInvalid extends errors_JOSEError {
    constructor(){
        super(...arguments);
...
const decrypt = async (enc, cek, ciphertext, iv, tag, aad)=>{
    ...
    if (!iv) {
        throw new errors_JWEInvalid("JWE Initialization Vector missing");
    }
    if (!tag) {
        throw new errors_JWEInvalid("JWE Authentication Tag missing");
    }
...
async function decrypt_flattenedDecrypt(jwe, key, options) {
    ...
    if (jwe.iv !== undefined && typeof jwe.iv !== "string") {
        throw new errors_JWEInvalid("JWE Initialization Vector incorrect type");
    }
    if (typeof jwe.ciphertext !== "string") {
        throw new errors_JWEInvalid("JWE Ciphertext missing or incorrect type");
    }
    if (jwe.tag !== undefined && typeof jwe.tag !== "string") {
        throw new errors_JWEInvalid("JWE Authentication Tag incorrect type");
    }
...
async function calculateJwkThumbprint(jwk, digestAlgorithm) {
    ...
    const data = buffer_utils_encoder.encode(JSON.stringify(components));
...
`;
    return jsxRuntime_module_o(preact_module_p, {
        children: jsxRuntime_module_o("script", {
            dangerouslySetInnerHTML: {
                    ...
                    providers.map((provider, i)=>{
                        ...
                        return jsxRuntime_module_o("div", {
                            ...
                            children: [
                                ...
                                (provider.type === "email" || provider.type === "credentials" || provider.type === "webauthn") && i > 0 && providers[i - 1].type !== "email" && providers[i - 1].type !== "credentials" && providers[i - 1].type !== "webauthn" && jsxRuntime_module_o("hr", {}),
                                ...
                                provider.type === "webauthn" && jsxRuntime_module_o("form", {
                                    action: provider.callbackUrl,
                                    method: "POST",
                                    id: `${provider.id}-form`,
                                    children: [
                                ...
                                (provider.type === "email" || provider.type === "credentials" || provider.type === "webauthn") && i + 1 < providers.length && jsxRuntime_module_o("hr", {})
                            ]
                        }, provider.id);
                    })
                ]
...
function send({ html, title, status, cookies, theme, headTags }) {
    return {
        ...
        body: `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>${styles}</style><title>${title}</title>${headTags ?? ""}</head><body class="__next-auth-theme-${theme?.colorScheme ?? "auto"}"><div class="page">${dist_k(html)}</div></body></html>`
    };
}
/**
...
const webauthnChallenge = {
     ...
     */ async use (options, cookies, resCookies) {
        ...
        if (!value?.value) throw new InvalidCheck("Challenge value could not be parsed.");
        // Clear the pkce code verifier cookie after use
        const cookie = {
...
/** Set default env variables on the config object */ function setEnvDefaults(envObject, config) {
    ...
    config.trustHost ?? (config.trustHost = !!(envObject.AUTH_URL ?? envObject.AUTH_TRUST_HOST ?? envObject.VERCEL ?? envObject.NODE_ENV !== "production"));
...
var server = __webpack_require__(951);
...
/** If `NEXTAUTH_URL` or `AUTH_URL` is defined, override the request's URL. */ function reqWithEnvURL(req) {
    ...
    return new server.NextRequest(href.replace(origin, envOrigin), req);
}
/**
 ...
 */ function env_setEnvDefaults(config) {
    try {
        const url = process.env.AUTH_URL ?? process.env.NEXTAUTH_URL;
        if (!url) return;
        config.basePath || (config.basePath = new URL(url).pathname);
    } catch  {} finally{
...
function cookies() {
    if (static_generation_bailout_staticGenerationBailout("cookies", {
        ...
        return RequestCookiesAdapter.seal(new spec_extension_cookies/* RequestCookies */.q(new Headers({})));
    }
    const requestStore = request_async_storage_external_requestAsyncStorage.getStore();
    if (!requestStore) {
...
async function actions_signIn(provider, options = {}, authorizationParams, config) {
    ...
    const signInURL = actions_createActionURL("signin", headers, config.basePath);
    if (!provider) {
...
async function actions_signOut(options, config) {
    const headers = new Headers(headers_headers());
    headers.set("Content-Type", "application/x-www-form-urlencoded");
    const url = actions_createActionURL("signout", headers, config.basePath);
...
async function update(data, config) {
    ...
    const url = actions_createActionURL("session", headers, config.basePath);
 ...
 */ function actions_createActionURL(action, h, basePath) {
...
async function handleAuth(args, config, userMiddlewareOrRoute) {
    ...
    if (authorized instanceof Response) {
        ...
        await userMiddlewareOrRoute(augmentedReq, args[1]) ?? server.NextResponse.next();
    } else if (!authorized) {
        ...
        if (request.nextUrl.pathname !== signInPage) {
            ...
            response = server.NextResponse.redirect(signInUrl);
        }
    }
...
"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   E: () => (/* binding */ ImageResponse)
/* harmony export */ });
/**
...
/* harmony import */ var _error__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(2);
...
"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   Nf: () => (/* binding */ userAgentFromString),
/* harmony export */   WE: () => (/* binding */ userAgent)
/* harmony export */ });
/* unused harmony export isBot */
/* harmony import */ var next_dist_compiled_ua_parser_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(254);
...
const _context = __webpack_require__(798);
const reader = {
    url (req) {
        return req.url;
    },
...
const serverExports = {
    NextRequest: (__webpack_require__(382)/* .NextRequest */ .I),
    NextResponse: (__webpack_require__(651)/* .NextResponse */ .x),
    ImageResponse: (__webpack_require__(289)/* .ImageResponse */ .E),
    userAgentFromString: (__webpack_require__(594)/* .userAgentFromString */ .Nf),
    userAgent: (__webpack_require__(594)/* .userAgent */ .WE)
};
